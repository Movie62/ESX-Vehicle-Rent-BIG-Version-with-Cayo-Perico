...
ESX.ShowNotification("~g~Mietzeitraum zuende")
end)
...
table.insert(elements, { label = "Bestätigen", value = "rentButton" })
...
ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehiclesRentrent_this_veh',
	...
	function(data, menu)
        ...
        { 
          title = 'Mietzeit bestätigen',
          align = 'top-left',
          elements = {
            {label = '<span style="color:Yellow;">Mieten für</span> <span style="color:Green;">' .. curRentPrice  .. '$ ?</span>', value = 'lol'},
			{label = '<span style="color:Green;">Bestätigen</span>', value = 'yes'},
        ...
        function (data2, menu2)
			...
			ESX.TriggerServerCallback('esx_vehiclesRent:buyVehicle', function(isBuyed)
				...
				ESX.ShowNotification("~g~Du hast das gewählte Fahrzeug erfolgreich gemietet.")
				else 
				ESX.ShowNotification("~r~Du hast nicht genug Geld dabei!")
				end
			end, vehicle)
			end
	  ...
	  elseif not price then 
        ...
        { 
          ...
          align = 'top-left',
          elements = {
            {label = '<span style="color:Yellow;">Fahrzeug mieten?</span>', value = 'lol'},
			{label = '<span style="color:Green;">Bestätigen</span>', value = 'yes'},
            {label = 'Back', value = 'no'},
          },
        },
        function (data2, menu2)
			...
			ESX.TriggerServerCallback('esx_vehiclesRent:getVehicle', function()
				...
				ESX.ShowNotification("~g~Du hast das Fahrzeug erfolgreich gemietet")
			end, vehicle)
			end
			if data2.current.value == 'no' then
			menu2.close()
        ...
        function (data2, menu2)
	  	...
	  	ESX.ShowNotification("~r~Du kannst kein Fahrzeug für 0 Minuten mieten.")
	  end
	  end
	...
	while true do
		Citizen.Wait(0)
			...
			for i = 1, #spawnedVehs, 1 do
				...
				AddTextComponentString('Drücke ~INPUT_CONTEXT~ um das Fahrzeug zu mieten.')
				DisplayHelpTextFromStringLabel(0, 0, 1, -1)
				if IsControlJustPressed(0, 38) then
					if rentNoJob >= Config.MaxNoJob then 
					ESX.ShowNotification("Du kannst nicht mehr als " .. Config.MaxNoJob .. " Fahrzeuge mieten!")
					else 
				...
				if rentJob >= Config.MaxJob then 
					ESX.ShowNotification("Du kannst nicht mehr als ~y~" .. Config.MaxJob .. " Fahrzeuge mieten!")
					else 
					OpenVehicleMenu(spawnedVehs[i])
				end
				end
				...
				founded = true
				SetTextComponentFormat('STRING')
				AddTextComponentString('Du kannst dieses Fahrzeug nicht mieten!')
				DisplayHelpTextFromStringLabel(0, 0, 1, -1)
				end
				end
				end
